        ...
        private void FixedUpdate()
        {
            ...
            if (Physics.SphereCast(_camTransform.position, 2.5f, _camTransform.TransformDirection(_camTransform.forward), out hit, 5.0f, 1 << 10))
            {
                ...
                if (_objectToAddToInventory.TryGetComponent(out Object obj))
                {
                    ...
                    _objectToAddToInventory.GetComponentInChildren<Outline>().enabled = true;    // Get the outline script and enable it
                }
            }
            else
            {
                if (_objectToAddToInventory)
                {
                    if (_objectToAddToInventory.TryGetComponent(out Object obj))
                    {
                        ...
                        _objectToAddToInventory.GetComponentInChildren<Outline>().enabled = false;;  // Get the outline script and disable it
